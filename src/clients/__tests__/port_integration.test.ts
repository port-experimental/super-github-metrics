...
describe('Port Client Integration', () => {
  ...
  describe('API Request Handling', () => {
    ...
    it('should handle non-401 errors without retry', async () => {
      ...
      await expect(client.get('/entities')).rejects.toEqual({ response: { status: 500 } });
  ...
  describe('Entity Operations', () => {
    it('should upsert entity properties', async () => {
      ...
      mockAxios.post.mockResolvedValueOnce({ data: mockOAuthResponse });
      mockAxios.patch.mockResolvedValueOnce(mockApiResponse);
  ...
  describe('Static Methods', () => {
    ...
    it('should handle concurrent static method calls', async () => {
      ...
      const promises = [
        PortClient.upsertProps('github_user', 'user1', { name: 'test1' }),
        PortClient.upsertProps('github_user', 'user2', { name: 'test2' }),
        PortClient.upsertProps('github_user', 'user3', { name: 'test3' }),
      ];
